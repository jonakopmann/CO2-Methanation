...
def get_S_co2(T):
    ...
    E = -0.158558
    G = 172.707974
    return A * math.log(t) + B * t + (C / 2) * t ** 2 + (D / 3) * t ** 3 - E / (2 * t ** 2)
...
class ReactionRate:
    ...
    def get_K_eq(self, T):
        ...
        H = (self.params.v_co2 * H_f_co2 + self.params.v_h2 * H_f_h2
             + self.params.v_ch4 * H_f_ch4 + self.params.v_h2o * H_f_h2o)  # [kJ/mol]
        ...
        G = (1000 * H - T * S)  # [J/mol]
        #print(math.exp(-G / (self.params.R * T)))
    ...
    def calc(self, y_co2, y_h2, y_ch4, y_h2o):
        ...
        return (self.get_k() * (p_h2 ** 0.5) * (p_co2 ** 0.5) * (
                1 - (p_ch4 * (p_h2o ** 2)) / (p_co2 * (p_h2 ** 4) * self.get_K_eq(self.params.T_ref)))
                / ((1 + self.get_K_oh() * (p_h2o / (p_h2 ** 0.5)) + self.get_K_h2() * (
                        p_h2 ** 0.5) + self.get_K_mix() * (p_co2 ** 0.5)) ** 2))
