...
class Integrator:
    ...
    def ode_y_i(self, y_i, y_i_surf, v_i, r, y_co2, y_h2, y_ch4, y_h2o, T):
        return ((1 / self.params.epsilon) * self.diff.calc(y_i, y_i_surf, r)
                ...
                * v_i * self.r_rate.calc(y_co2, y_h2, y_ch4, y_h2o, T))
    ...
    def run(self):
        ...
        alg_co2 = (self.params.y_co2_0
                   + self.params.delta_y * ca.sin(2 * ca.pi * self.params.f * t) - y_co2_surf)
        alg_h2 = (self.params.y_h2_0
                  - self.params.delta_y * ca.sin(2 * ca.pi * self.params.f * t) - y_h2_surf)
        alg_ch4 = (self.params.y_ch4_0
                   + 0 * self.params.delta_y * ca.sin(2 * ca.pi * self.params.f * t) - y_ch4_surf)
        alg_h2o = (self.params.y_h2o_0
                   + 0 * self.params.delta_y * ca.sin(2 * ca.pi * self.params.f * t) - y_h2o_surf)
        alg_T = self.params.T_0 + self.params.delta_T * self.params.T_0 * ca.sin(2 * ca.pi * self.params.f * t) - T_surf

        # assign equations to ode for each radius r
        for r in range(self.params.r_steps):
            ode_co2[r] = self.ode_y_i(y_co2, y_co2_surf, self.params.v_co2, r,
                                      y_co2[r], y_h2[r], y_ch4[r], y_h2o[r], T[r])
            ode_h2[r] = self.ode_y_i(y_h2, y_h2_surf, self.params.v_h2, r,
                                     y_co2[r], y_h2[r], y_ch4[r], y_h2o[r], T[r])
            ode_ch4[r] = self.ode_y_i(y_ch4, y_ch4_surf, self.params.v_ch4, r,
                                      y_co2[r], y_h2[r], y_ch4[r], y_h2o[r], T[r])
            ode_h2o[r] = self.ode_y_i(y_h2o, y_h2o_surf, self.params.v_h2o, r,
                                      y_co2[r], y_h2[r], y_ch4[r], y_h2o[r], T[r])
            ode_T[r] = (1 / (self.params.roh_s * self.params.c_p) * self.h_conduction.calc(T, T_surf, r)
                        - (1 - self.params.epsilon) / self.params.c_p * self.r_rate.get_H_R(T[r])
                        * self.r_rate.calc(y_co2[r], y_h2[r], y_ch4[r], y_h2o[r], T[r]))

        # create integrator
        dae = {
            'x': ca.veccat(y_co2, y_h2, y_ch4, y_h2o, T),
            'z': ca.vertcat(y_co2_surf, y_h2_surf, y_ch4_surf, y_h2o_surf, T_surf),
            't': t,
            'ode': ca.vertcat(ode_co2, ode_h2, ode_ch4, ode_h2o, ode_T),
            'alg': ca.vertcat(alg_co2, alg_h2, alg_ch4, alg_h2o, alg_T)
        }
        ...
        # integrate
        res = integrator(x0=x0, z0=ca.vertcat(self.params.y_co2_0, self.params.y_h2_0, self.params.y_ch4_0,
                                                  self.params.y_h2o_0, self.params.T_0))
